 ...
 * default values. The load function will print if a parameter is loaded from
 * the server or if its default value is used. In each case, the final value
 * is printed, too.
 *
 * The main #load function can handle bool, int, double and string - all as
 * single values and as vectors. Also matrices of double (float) can be loaded.
 *
 * Additionaly, there are two more dedicated load functions: #load_path and
 * #load_topic. Both are basically loading a simple string from the parameter
 * server as descript above. Furthermore the strings are specificly manipulated:
 *  + load_path: Every "$(find xyz)" will be replaced with the path of
 *    package xyz.
 *  + load_topic: Names are resolved relativ to current namespace or node.
 *    "~xyz" will resolve to "/ns/current_node/xyz" and "/xyz" will resolve to
 *    "/ns/xyz" (not in namespace of current node).
 *
 * \section example_sec Example Output
 *
